        ...
        <AnimatePresence>
          {showSuggestions && query.trim() && suggestions.length > 0 && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 8 }}
              className="absolute left-0 right-0 mt-2 z-50 rounded-2xl bg-[#090a0f]/95 border border-white/10 shadow-2xl overflow-hidden backdrop-blur-xl text-left p-1.5"
            >
              {suggestions.map((s) => (
                <button
                  key={s}
                  ...
                  className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs text-white/80 hover:text-white hover:bg-white/5 transition-colors font-medium group"
                >
                  <span>{s}</span>
